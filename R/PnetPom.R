...
## locked and loaded.
## setClassUnion("net.bridge","PomNet")
## setIs("net.bridge","Pnet")
## setMethod("as.Pnet","PomNet",function(x) x)
## setMethod("is.Pnet","PomNet",function(x) TRUE)


## as.Pnet.PomNet <- function (x) {
##   if (!("Pnet" %in% class(x)))
##     class(x) <- c(class(x),"Pnet")
##   x
## }

setMethod("PnetPriorWeight","PomNet", function (net) {
    stop("Function Not Implemented")
  NetworkUserObj(net,"priorWeight")
})

setMethod("PnetPriorWeight<-","PomNet", function (net,value) {
    stop("Function Not Implemented")
  NetworkUserObj(net,"priorWeight") <- value
  invisible(net)
})

setMethod("PnetPnodes","PomNet", function (net) {
    stop("Function Not Implemented")
  NetworkNodesInSet(net,"pnodes")
})
setMethod("PnetPnodes<-","PomNet", function (net, value) {
    stop("Function Not Implemented")
...
##'Creates a PomNet object which is also a Pnet
##'
##'
...
##'
##'These correspond to fields in the \code{\link{RNetica}{PomNet}} object.
##'
...
##'@return
##'
##'An object of class \code{\link[RNetica]{PomNet}} which is also in the
...
##'\code{\link[RNetica]{PomNet}}, \code{\link[RNetica]{IDname}}
##'
##'Peanut Package: \code{\link[Peanut]{Warehouse}},
##'\code{\link[Peanut]{WarehouseMake}}
##'
##'PPomegranite Pacakge \code{\link{BNWarehouse}}
...
##'startSession(sess)
##'
##'anet <- MakePnet.PomNet(sess,"Anet",
...
##'miniPP <- MakePnet.PomNet(sess,"miniPP",netman1[1,,drop=FALSE])
##'
##'DeleteNetwork(miniPP)
##'stopSession(sess)
##'
##'
##'@export MakePnet.PomNet
MakePnet.PomNet <-function (sess,name,data) {
    stop("Function Not Implemented")
...
Free.PomNet <- function (obj) {
    stop("Function Not Implemented")
    invisible(NULL)}


Save.PomNet <- function (net,pathname) {
    stop("Function Not Implemented")
...
Reload.PomNet <- function (net,pathname) {
    stop("Function Not Implemented")
  if (missing(pathname) || is.null(pathname))
    pathname <- PnetPathname(net)
  DeleteNetwork(net)
...
## deleted later
setMethod("PnetMakeStubNodes","PomNet", function (net,nodes) {
    stop("Function Not Implemented")
...
setMethod("PnetRemoveStubNodes","PomNet", function (net,nodes) {
    stop("Function Not Implemented")
  DeleteNodes(nodes)
})
...
##'
##'@aliases PnetAdjoin,PomNet-method PnetDetach,PomNet-method
...
##'startSession(sess)
##'
##'PM <- ReadNetworks(file.path(library(help="PPomegranite")$path, "testnets",
##'     "miniPP-CM.dne"), session=sess)
##'EM1 <- ReadNetworks(file.path(library(help="PPomegranite")$path, "testnets",
...
##'
##'@exportMethod
setMethod("PnetAdjoin","PomNet", function (hub, spoke) {
    stop("Function Not Implemented")
  AdjoinNetwork(hub,spoke,paste("Spoke",NetworkName(spoke),sep="_"))
})

setMethod("PnetDetach","PomNet", function (motif, spoke) {
    stop("Function Not Implemented")
...
##'Properties of class \code{"PomNode"} as a \code{"Pnode"}
##'
##'
...
##'@aliases Pnode.Properties as.Pnode,PomNode-method
##'is.Pnode,PomNode-method Pnode,PomNode-method
##'PnodeLnAlphas,PomNode-method PnodeLnAlphas<-,PomNode-method
##'PnodeBetas,PomNode-method PnodeBetas<-,PomNode-method
##'PnodeQ,PomNode-method PnodeQ<-,PomNode-method
##'PnodeRules,PomNode-method PnodeRules<-,PomNode-method
##'PnodeLink,PomNode-method PnodeLink<-,PomNode-method
##'PnodeLinkScale,PomNode-method PnodeLinkScale<-,PomNode-method
##'PnodePostWeight,PomNode-method PnodePriorWeight,PomNode-method
##'PnodePriorWeight<-,PomNode-method
##'@docType class
##'@note
##'
...
## setClassUnion("node.bridge","PomNode")
## setIs("node.bridge","Pnode")

setMethod("as.Pnode","PomNode",function(x) {
    stop("Function Not Implemented")
  NodeSets(x) <- union("pnodes",NodeSets(x))
  if (is.na(NodeUserField(x,"Truename")))
    NodeUserField(x,"Truename") <- NodeName(x)
  x})
setMethod("is.Pnode","PomNode",function(x)
    stop("Function Not Implemented")
  "pnodes" %in% NodeSets(x)
  )


## as.Pnode.PomNode <- function (x) {
##   if (!("Pnode" %in% class(x)))
##     class(x) <- c(class(x),"Pnode")
##   x
## }

setMethod("PnodeNet","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeNet(node)
})

setMethod("PnodeRules","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeUserObj(node,"rules")
})

setMethod("PnodeRules<-","PomNode", function (node,value) {
    stop("Function Not Implemented")
  NodeUserObj(node,"rules") <- value
  node
})

setMethod("PnodeLink","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeUserObj(node,"link")
})

setMethod("PnodeLink<-","PomNode", function (node,value) {
    stop("Function Not Implemented")
  NodeUserObj(node,"link") <- value
  node
})

setMethod("PnodeQ","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeUserObj(node,"Q")
})

setMethod("PnodeQ<-","PomNode", function (node,value) {
    stop("Function Not Implemented")
  NodeUserObj(node,"Q") <- value
  node
})

setMethod("PnodeLnAlphas","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeUserObj(node,"lnAlphas")
})

setMethod("PnodeLnAlphas<-","PomNode", function (node,value) {
    stop("Function Not Implemented")
  NodeUserObj(node,"lnAlphas") <- value
  node
})

setMethod("PnodeBetas","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeUserObj(node,"betas")
})

setMethod("PnodeBetas<-","PomNode", function (node,value) {
    stop("Function Not Implemented")
...
setMethod("PnodeLinkScale","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeUserObj(node,"linkScale")
})

setMethod("PnodeLinkScale<-","PomNode", function (node,value) {
    stop("Function Not Implemented")
  NodeUserObj(node,"linkScale") <- value
  node
})

setMethod("PnodePriorWeight","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeUserObj(node,"priorWeight")
})

setMethod("PnodePriorWeight<-","PomNode", function (node,value) {
    stop("Function Not Implemented")
  NodeUserObj(node,"priorWeight") <- value
  node
})

setMethod("PnodePostWeight","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeExperience(node)
})

setMethod("PnodeProbs","PomNode", function (node) {
    stop("Function Not Implemented")
  NodeProbs(node)
})

setMethod("PnodeProbs<-","PomNode", function (node,value) {
    stop("Function Not Implemented")
...
##'allows the states of a \code{\link[RNetica]{PomNode}} to be associated
...
##'not true.
##'
##'@aliases PnodeParentTvals,PomNode-method
##'@param node A \code{\link{Pnode}} which is also a
##'\code{\link[RNetica]{PomNode}}.
##'@return
##'
...
##'@author Russell Almond
##'@seealso \code{\link{Pnode.PomNode}}, \code{\link[Peanut]{Pnode}},
##'\code{\link[CPTtools]{effectiveThetas}},
##'\code{\link{BuildTable,PomNode-method}},
##'\code{\link{maxCPTParam,PomNode-method}}
##'@references
##'
...
##'stopSession(sess)
##'@exportMethod
setMethod("PnodeParentTvals","PomNode", function (node) {
    stop("Function Not Implemented")
  lapply(NodeParents(node),PnodeStateValues)
})

setMethod("Pnode","PomNode",
...
##'(\link[RNetica]{Extract.PomNode}) operator.
##'
...
##'
##'@aliases BuildTable,PomNode-method
##'@param node A \code{\link[Peanut]{Pnode}} and
##'\code{\link[RNetica]{PomNode}} object whose table is to be built.
##'@return
##'
...
##'\code{\link[RNetica]{Extract.PomNode}} (\code{[}) %]
##'@references
##'
##'Almond, R. G. (2015) An IRT-based Parameterization for Conditional
##'Probability Tables.  Paper presented at the 2015 Bayesian Application
...
##'
##' @exportMethod 
setMethod("BuildTable","PomNode", function (node) {
    stop("Function Not Implemented")
...
##'Calculates the log likelihood for a set of data under a Pnet.PomNet model
##'
##'
##'The method \code{calcPnetLLike.PomNet} calculates the log likelihood for a
##'set of data contained in \code{cases} using the current conditional
##'probability tables in a \code{\link{Pnet.PomNet}}.  Here \code{cases}
...
##'algorithm.  The \code{\link{Pnet.PomNet}} represents a model (with
...
##'
##'@aliases calcPnetLLike,PomNet-method
##'@param net A \code{\link{Pnet.PomNet}} object representing a parameterized
##'network.
...
##'@seealso
##'
##'\code{\link[Peanut]{Pnet}}, \code{\link{Pnet.PomNet}},
...
##'startSession(sess)
##'
##'irt10.base <- ReadNetworks(file.path(library(help="PPomegranite")$path,
...
##'
##'casepath <- file.path(library(help="PPomegranite")$path,
...
##'
##' @exportMethod
setMethod("calcPnetLLike","PomNet", function (net,cases){
    stop("Function Not Implemented")
  llike <- 0
  nextRec <- "FIRST"
...
##'Calculate expected tables for a Pnet.PomNet
##'
##'
...
##'\code{\link{Pnode.PomNode}} should be the mean of the Dirichlet
...
##'
##'@aliases calcExpTables,PomNet-method
##'@param net A \code{\link{Pnet.PomNet}} object representing a parameterized
##'network.
##'@param cases A character scalar giving the file name of a Netica case file
##'(see \code{\link[RNetica]{write.CaseFile}}).
...
##'
##' @exportMethod 
setMethod("calcExpTables","PomNet", function (net, cases, Estepit=1,
...
##'Find optimal parameters of a Pnode.PomNode to match expected tables
##'
##'
...
##'
##'
##'This method is called on on a \code{\link{Pnode.PomNode}} object during
...
##'test.
##'
##'@aliases maxCPTParam,PomNode-method
...
##'@seealso
##'
##'\code{\link[Peanut]{Pnode}}, \code{\link{Pnode.PomNode}},
...
##'}
##'
##'casepath <- paste(library(help="PPomegranite")$path,
##'                           "testdat","IRT10.2PL.200.items.cas",
##'                           sep=.Platform$file.sep)
##'
##'
...
##'
##'@exportMethod
setMethod("maxCPTParam","PomNode", function (node, Mstepit=5,
                                    tol=sqrt(.Machine$double.eps)) {
    stop("Function Not Implemented")
...
Free.PomNode <-function (obj) {
    stop("Function Not Implemented")
  invisible(NULL)
}

Delete.PomNode <- function (obj) {
    stop("Function Not Implemented")
  DeleteNodes(obj)
}
...
##'
##'@param net A \code{\link[RNetica]{PomNet}} object in which the node will
##'be created.
...
##'@return
##'
##'An object of class \code{\link[RNetica]{PomNode}} which is also in the
...
##'@author Russell Almond
##'@seealso RNetica Package: \code{\link[RNetica]{PomNode}},
...
##'\code{\link[Peanut]{WarehouseMake}}
##'
##'PPomegranite Pacakge \code{\link[Peanut]{PnodeWarehouse}}
...
##'
##'Physics <- MakePnode.PomNode(miniPP,"Physics",phys.dat)
##'
##'## Continuous Example
##'dur.dat <- nodeman1[nodeman1$NodeName=="Duration",]
##'
##'Duration <- MakePnode.PomNode(miniPP,"Duration",dur.dat)
##'
##'
##'DeleteNetwork(miniPP)
##'stopSession(sess)
##'
##'
##'@export MakePnode.PomNode
MakePnode.PomNode <- function (net, name, data) {
    stop("Function Not Implemented")
